	...
	public function generate($_FPOST)
	{
		...
		for ($i=0; $i < count($column); $i++) {
			...
			if($column[$i] == trim($primaryKey)){
				$htmlInputs .=' 							<input type="hidden" value="<?= $'.$inputName.'?>" id="'.$inputName.'" name="'.$inputName.'" class="form-control" placeholder="'. ucwords($inputlabel).' ...'.'"'.$inputMaxlength.' '.$inputRequired.'>'."\n";
				$ciValidationType = '|numeric';														

			}elseif($iType[$i] == '1' && $crudShow == '1') { 
				$htmlInputs .= '								<div class="form-group">
									<label for="'.$inputName.'">'.ucwords($inputlabel).': '.$htmlInputRequired.'</label>
									<input type="text" value="<?= $'.$inputName.'?>" id="'.$inputName.'" name="'.$inputName.'" class="form-control" placeholder="'. ucwords($inputlabel).' ...'.'"'.$inputMaxlength.' '.$inputRequired.'>
								</div>'."\n";
			}elseif($iType[$i] == '2' && $crudShow == '1') {
				$htmlInputs .= '								<div class="form-group">
									<label for="'.$inputName.'">'.ucwords($inputlabel).': '.$htmlInputRequired.'</label>
									<input type="number" value="<?= $'.$inputName.'?>" id="'.$inputName.'" name="'.$inputName.'" class="form-control" placeholder="'. ucwords($inputlabel).' ...'.'"'.$inputMaxlength.' number="true" '.$inputRequired.'>
								</div>'."\n";	
				$ciValidationType = '|numeric';														
			}elseif($iType[$i] == '3' && $crudShow == '1') {
				$htmlInputs .= '								<div class="form-group">
									<label for="'.$inputName.'">'.ucwords($inputlabel).': '.$htmlInputRequired.'</label>
									<input type="password" value="<?= $'.$inputName.'?>" id="'.$inputName.'" name="'.$inputName.'" class="form-control" placeholder="'. ucwords($inputlabel).' ...'.'"'.$inputMaxlength.' '.$inputRequired.'>
								</div>'."\n";
			}elseif($iType[$i] == '4' && $crudShow == '1') {
				$htmlInputs .= '								<div class="form-group">
									<label for="'.$inputName.'">'.ucwords($inputlabel).': '.$htmlInputRequired.'</label>
									<input type="email" value="<?= $'.$inputName.'?>" id="'.$inputName.'" name="'.$inputName.'" class="form-control" placeholder="'. ucwords($inputlabel).' ...'.'"'.$inputMaxlength.' '.$inputRequired.'>
								</div>'."\n";										
			}elseif($iType[$i] == '5' && $crudShow == '1') {
				$htmlInputs .= '								<div class="form-group">
									<label for="'.$inputName.'">'.ucwords($inputlabel).': '.$htmlInputRequired.'</label>
									<textarea cols="40" rows="5" value="<?= $'.$inputName.'?>" id="'.$inputName.'" name="'.$inputName.'" class="form-control" placeholder="'. ucwords($inputlabel).' ...'.'"'.$inputMaxlength.' '.$inputRequired.'></textarea>
								</div>'."\n";					
			}elseif($iType[$i] == '6' && $crudShow == '1') {
				$htmlInputs .= '								<div class="form-group">
									<label for="'.$inputName.'">'.ucwords($inputlabel).': '.$htmlInputRequired.'</label>
									<select value="<?= $'.$inputName.'?>" id="'.$inputName.'" name="'.$inputName.'" class="custom-select" '.$inputRequired.'>
				...
				$htmlInputs .= '								<div class="form-group">
									<label for="'.$inputName.'">'.ucwords($inputlabel).': '.$htmlInputRequired.'</label>
									<input type="date" value="<?= $'.$inputName.'?>" id="'.$inputName.'" name="'.$inputName.'" class="form-control" dateISO="true" '.$inputRequired.'>
								</div>'."\n";					
				$ciValidationType = '|valid_date';								
			}else{
				$htmlInputs .= ''."\n"; $htmlInputs .=' 							<input type="text" value="<?= $'.$inputName.'?>" id="'.$inputName.'" name="'.$inputName.'" class="form-control" placeholder="'. ucwords($inputlabel).' ...'.'"'.$inputMaxlength.' '.$inputRequired.'>'."\n";					
			}
